...
int main(int argc, char *argv[])
{
    ...
    hyperedge_weights[1] = 10;
    hyperedge_weights[2] = 1;
    hyperedge_weights[3] = 10;

    // Construct hypergraph for DEFAULT preset
    mt_kahypar_hypergraph_t hypergraph = mt_kahypar_create_hypergraph(
        DEFAULT, num_nodes, num_hyperedges, hyperedge_indices.get(), hyperedges.get(),
        hyperedge_weights.get(), node_weights.get());

    std::cout << "Number of Nodes            = " << mt_kahypar_num_hypernodes(hypergraph)
              << std::endl;
    std::cout << "Number of Hyperedges       = " << mt_kahypar_num_hyperedges(hypergraph)
              << std::endl;
    std::cout << "Number of Pins             = " << mt_kahypar_num_pins(hypergraph)
              << std::endl;
    std::cout << "Total Weight of Hypergraph = "
              << mt_kahypar_hypergraph_weight(hypergraph) << std::endl;
