...
int main(int argc, const char *argv[]) {
    ...
    { // header
        ...
        sstream >> matrix_market >> object >> matrix_format >> data_format >>
            symmetry_str;
    ...
    { // dimensions
        ...
        if(nrows != ncols) {
            std::cerr << "Num Rows != Num Cols --> This doesn't work for graphs."
                      << std::endl;
            std::abort();
        }
        num_nodes = nrows;
    ...
    for(uint64_t e = 0; e < nnz; ++e) {
        ...
        if(row >= num_nodes || col >= num_nodes) {
            std::cerr << "Row or col index higher than number of nodes " << row << " "
                      << col << " " << num_nodes << std::endl;
