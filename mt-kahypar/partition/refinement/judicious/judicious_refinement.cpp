  ...
  void JudiciousRefiner::doRefinement(PartitionedHypergraph& phg, const PartitionID part_id) {
    ...
    auto delta_func = [&](const HyperedgeID he,
      ...
      if (pin_count_in_from_part_after == 1) {
        for (HypernodeID v : phg.pins(he)) {
          if (phg.partID(v) == part_id) {
            // NOTE: hopefully this doesnt happen too often for the same node in one round <2022-05-21, noahares>
            _pq.increaseBenefit(v, phg.edgeWeight(he), part_id);
            // can abort because only one node satisfies this condition
            break;
          }
        }
      }
    ...
    while (_pq.findNextMove(phg, move)) {
      ...
      if (_moves.size() > _refinement_nodes.size() * _context.refinement.judicious.abort_factor) {
        ...
        break;
      } else if (_part_loads.topKey() >= from_load * _context.refinement.judicious.part_load_margin /* || _part_loads.topKey() >= _part_loads.keyOfSecond() * _context.refinement.judicious.part_load_margin */) break;
      updateNeighbors(phg, move);
      _pq.updatePartLoads(phg, move.from, move.to);
    }
    ...
    for (const HyperedgeID& he : _edges_with_gain_changes) {
      for (const HypernodeID& v : phg.pins(he)) {
          if (phg.partID(v) == move.from) {
            ...
            _gain_changes[v] += phg.edgeWeight(he);
          }
      }
    }
    for (const auto& v : _nodes_with_gain_update) {
      _pq.decreasePenalty(v, _gain_changes[v], move.to);
    }
    _edges_with_gain_changes.clear();
